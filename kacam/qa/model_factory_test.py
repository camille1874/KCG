...
class _EntAttention(Layer):
    def __init__(self, ent_dim):
        self.ent_dim = ent_dim
        self.ent_sim_weight = K.variable(np.random.uniform(-1.0, 1.0, size=(ent_dim, ent_dim)), dtype="float32",name="ent_sim_weight")
    ...
    def __call__(self, x):
        ...
        sen_ent2 = x[1]
        def _outer(AB):
            s_e_1 = AB[0] # (batch, enc(dec), ent_dim)
            s_e_2 = AB[1] # (batch, dec(enc), ent_dim)
            weight_s_e_2 = K.dot(s_e_2, self.ent_sim_weight)  # (batch, dec(enc), ent_dim)
            return K.tanh(K.batch_dot(s_e_1, K.permute_dimensions(weight_s_e_2, (0, 2, 1))))  # (batch, enc(dec), dec(enc))

        len_x = K.int_shape(sen_ent1)[1] # enc(dec)
        len_y = K.int_shape(sen_ent2)[1] # dec(enc)
        return Lambda(_outer, output_shape=(len_x,len_y,))([sen_ent1, sen_ent2])
...
class ModelFactory(object):
    @staticmethod
    def get_basic_model(model_param, embedding_file, ent_embedding_file, WordAttend, EntAttend, WordSoftAlign, EntSoftAlign, cxt_weight_mixed=True, ent_weight_mixed=True, entity_compare=True):
        ...
        question_ent_emb_mean = Lambda(lambda x:K.mean(x, axis=2), output_shape=(model_param.enc_timesteps, entity_dim,))(question_ent_emb)  # (batch, enc, ent_dim)
        answer_ent_emb_mean = Lambda(lambda x:K.mean(x, axis=2), output_shape=(model_param.dec_timesteps, entity_dim,))(answer_ent_emb)  # (batch, dec, ent_dim)

        # =======================================================================================================
        # ======================================= Entity Attention Layer ========================================
        # =======================================================================================================
        que_ent_atten_metrics = EntAttend([question_ent_emb_mean, answer_ent_emb_mean])  # (batch, enc, dec)
        ans_ent_atten_metrics = EntAttend([answer_ent_emb_mean, question_ent_emb_mean])  # (batch, dec, enc)
        ...
        # =======================================================================================================
        question_ent_align = EntSoftAlign(que_ent_atten_metrics, answer_ent_emb_mean, answer_len) # (batch, enc, ent_dim)
        answer_ent_align = EntSoftAlign(ans_ent_atten_metrics, question_ent_emb_mean, question_len, reverse=True) # (batch, dec, ent_dim)

        que_temp_cmp_ent_output = Multiply()([question_ent_emb_mean, question_ent_align])  # (batch, enc, ent_dim)
        ans_temp_cmp_ent_output = Multiply()([answer_ent_emb_mean, answer_ent_align])  # (batch, dec, ent_dim)
    ...
    @staticmethod
    def get_listwise_model(model_param, embedding_file, ent_embedding_file, WordAttend, EntAttend, WordSoftAlign, EntSoftAlign, cxt_weight_mixed=True, ent_weight_mixed=True, entity_compare=True):
        basic_model = ModelFactory.get_basic_model(model_param, embedding_file, ent_embedding_file, WordAttend, EntAttend, WordSoftAlign, EntSoftAlign, cxt_weight_mixed, ent_weight_mixed, entity_compare)
    ...
    def get_model(model_param, embedding_file, ent_embedding_file, model_type="listwise", cxt_weight_mixed=True, ent_weight_mixed=True, entity_compare=True):
        ...
        return ModelFactory.get_listwise_model(model_param, embedding_file, ent_embedding_file, WordAttend, EntAttend, WordSoftAlign, EntSoftAlign, cxt_weight_mixed=cxt_weight_mixed, ent_weight_mixed=ent_weight_mixed, entity_compare=entity_compare)
