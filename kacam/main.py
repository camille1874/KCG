...
from qa.model_factory_test import ModelFactory
...
parser = argparse.ArgumentParser(description='Knowledge-aware Attentive Compare-Aggregate Model')
...
def specific_load_epoch(model,epoch,prefix):
    #assert os.path.exists('best_model/wikiqa/'+prefix+'_weights_epoch_%s.h5' %
    #                      epoch), 'Weights at epoch %s not found' % epoch
    #filename = 'best_model/wikiqa/'+prefix+'_weights_epoch_%s.h5' % epoch
    assert os.path.exists('best_model/trecqa/'+prefix+'_weights_epoch_%s.h5' %
                          epoch), 'Weights at epoch %s not found' % epoch
    filename = 'best_model/trecqa/'+prefix+'_weights_epoch_%s.h5' % epoch
...
def main(args):
    ...
    def data_evaluate(epoch,small_evaluate_data,flag):
        c_1_j = 0        
        c_2_j = 0
        record_file = codecs.open("results" + flag + str(epoch) + "-" + datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S") + ".txt", encoding="utf-8", mode="w")
    ...
    if mode == "train":
        ...
        #specific_load_epoch(train_model,"14_wikiqa_listwise","train")
        #specific_load_epoch(predict_model,"14_wikiqa_listwise","train")
        specific_load_epoch(train_model,"28_trecqa_listwise","train")
        specific_load_epoch(predict_model,"28_trecqa_listwise","train")
        dev_map,dev_mrr = data_evaluate(epoch,dev_data,"dev")
        test_map,test_mrr = data_evaluate(epoch,test_data,"test")
        print("dev", dev_map, dev_mrr)
        print("test", test_map, test_mrr)
        return None
